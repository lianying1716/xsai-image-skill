...
const CLIENT_ID = "xsai-image-skill";
...
function normalizeImageError(error) {
  ...
  const known = new Set(["auth_required", "authorization_pending", "invalid_request", "invalid_scope", "unsupported_option", "model_unavailable", "not_found", "rate_limited", "timeout"]);
...
async function downloadJob(jobId, output, fetchImpl = globalThis.fetch) {
  ...
  const response = await fetchImpl(item.url, { signal: AbortSignal.timeout(60_000) });
  if (!response.ok) throw Object.assign(new Error("结果下载失败"), { code: `http_${response.status}` });
  let target = path.resolve(output || `image-${jobId}.png`);
  ...
  try {
    ...
    target = `${target.slice(0, -ext.length)}-${crypto.randomBytes(3).toString("hex")}${ext}`;
    await fs.writeFile(target, bytes, { flag: "wx" });
  }
...
async function main(argv = process.argv.slice(2), fetchImpl = globalThis.fetch) {
  ...
  if (args.command === "auth" && args.subcommand === "status") { const state = await readState(); return state ? { status: "configured", scopes: state.scopes || [], base_url: state.base_url } : { status: "signed_out" }; }
